...
class MedSegMLFlowHandler(MLFlowHandler):
    ...
    def _save_checkpoint(self, engine: Engine) -> None:
        ...
        if metric > self.best_metric:
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                save_path = os.path.join(tmp_dir, "model.pth")
                torch.save(self.save_dict, save_path)
                self.client.log_artifact(self.cur_run.info.run_id, save_path, "model")
    ...
    def _default_iteration_log(self, engine: Engine) -> None:
        super()._default_iteration_log(engine)
        assert not (hasattr(engine, "optimizer") and self.optimizer is not None)
